...
| 库存模块 | 编码·自行设计接口 | 客观行为测试（adapter 电池）+ 主观量表（接口类 5 维）（基线已建立：EXP-011） | `tasks/task-inventory-api.md` |
...
- 库存模块（EXP-011 首跑基线，n=1 单人/n=3 协作稿，探索性）：单人 22/25，协作稿 21/22/25，协作择优 25/25（技术负责人稿）→ 协作增益 **+13.6%**——自行设计接口编码任务（接口由执行者自定）：客观行为测试 4 稿全 33/33（功能趋同），但接口设计质量未趋同（盲评极差 4 分），专业角色（架构）产出显著优于通用单人；增益落在中等编码 +7% 与开放文案 +19% 之间，支持「协作价值 ∝ 开放性 × 复杂度」，详见 EXP-011-报告.md
